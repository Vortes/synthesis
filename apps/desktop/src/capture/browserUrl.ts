...
const FIREFOX_BASED_NAMES = new Set(["firefox", "zen", "waterfox", "librewolf"])
			...
			for (const win of windows) {
				...
				for (const tab of tabs) {
					...
					if (
						...
						// may return a suffix of the full page title (e.g. "Vortes/curate"
						// instead of "Comparing main...screenshot · Vortes/curate").
						...
						if (
							...
							console.log("[browserUrl] Matched tab by title:", lastEntry.title)
...
export function isBrowser(appName: string): boolean {
	return (
		appName in BROWSER_SCRIPTS || FIREFOX_BASED_NAMES.has(appName.toLowerCase())
	)
}
