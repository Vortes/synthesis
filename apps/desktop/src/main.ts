...
console.log(`[auth] CLERK_DOMAIN=${process.env.CLERK_DOMAIN}`)
console.log(`[auth] CLERK_AUTHORIZE_URL=${CLERK_AUTHORIZE_URL}`)
console.log(`[auth] CLERK_OAUTH_CLIENT_ID=${CLERK_OAUTH_CLIENT_ID}`)
...
function handleDeepLink(url: string) {
	console.log(`[auth] handleDeepLink called with: ${url}`)
	console.log(`[auth] pendingCodeVerifier exists: ${!!pendingCodeVerifier}`)
	console.log(`[auth] pendingState: ${pendingState}`)
	try {
		const parsed = new URL(url)
		console.log(`[auth] parsed hostname: ${parsed.hostname}`)
...
async function exchangeCodeForTokens(code: string) {
	try {
		...
		console.log(`[auth] Token exchange response: ${res.status}`)
		if (!res.ok) {
...
function initiateSignIn() {
	console.log("[auth] initiateSignIn called")
	pendingCodeVerifier = generateCodeVerifier()
	pendingState = generateState()
	console.log(`[auth] redirect_uri will be: ${PROTOCOL}://auth`)
	console.log(`[auth] authorize URL: ${CLERK_AUTHORIZE_URL}`)
...
app.on("open-url", (event, url) => {
	console.log(`[auth] open-url event received: ${url}`)
